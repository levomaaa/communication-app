...
def edit(message_id, edited_content):
    ...
    db.session.execute(text(sql), {"message_id":message_id, \
                                    "edited_content":edited_content, "user_role":user_role})
    db.session.commit()
    return True
...
def get_topic_id(message_id):
    login.check_role() 
    sql = "SELECT topic_id FROM adminmessages WHERE id=:message_id"
    result = db.session.execute(text(sql), {"message_id":message_id})
    return result.fetchone()

def get_message_count():
    login.check_role() 
    sql = "SELECT id, topic_id FROM adminmessages WHERE visible=TRUE"
